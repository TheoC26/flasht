...
export default function Home() {
  ...
  const MAX_SMALL_CARD_STACK_HEIGHT = 20;
  ...
  const CardStack = ({ pileName, cards, size = "md" }) => {
    ...
    return (
      ...
      >
        <AnimatePresence>
          {[...cards]
            .slice(0, MAX_SMALL_CARD_STACK_HEIGHT)
            .reverse()
            .map((card, i) => (
              <motion.div
                key={card.id}
                ...
                style={{
                  zIndex:
                    draggedCardRef.current?.id === card.id ? 9999 : i + 20,
                  y: -i * 10,
                  transform: `scale(${isSmall ? 1 : 1})`, // shrink cards in small piles
                  transformOrigin: "center bottom",
                }}
              >
                <Flashcard card={card} size={size} flipped={false} />
              </motion.div>
            ))}
        </AnimatePresence>
      </div>
  ...
  const ReverseCardStack = ({ pileName, cards, size = "md" }) => {
    const isSmall = size === "sm";
    return (
      <div
        id={`pile-${pileName}`}
  ...
  return (
    <main className="flex flex-col items-end justify-center min-h-screen p-10 bg-[#F1F1F1] overflow-hidden">
      <TopBar />
      <div className="w-full max-w-[100rem] mx-auto flex justify-center items-end">
        <div className="flex flex-col items-center mr-20 mb-5 relative">
          <ReverseCardStack pileName="main" size="md" cards={piles.main} />
          <div
          ...
          >
            ...
            <div className="flex gap-5 text-2xl mt-5">
              <button onClick={undo} className="bg-white rounded-2xl p-2 px-4 flashcard-shadow cursor-pointer transition-all hover:scale-105">
                Back
              </button>
              <button className="bg-[#CBF2CB] outline-2 outline-[#BFEBBF] rounded-2xl p-2 px-12 flashcard-shadow cursor-pointer transition-all hover:scale-105">
                Continue &gt;
              </button>
            </div>
          </div>
          ...
          <div
            className={`mt-10 flex gap-3 z-30 font-bold transition-all ${
        ...
        >
          ...
          <ReverseCardStack pileName="know" size="sm" cards={piles.know} />
          <div
            className={`absolute transition-all text-xl font-bold text-[#303030] ${
