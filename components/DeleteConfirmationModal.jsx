  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose}>
      ...
      <p className="text-md text-[#6D6D6D]">
        ...
        {item?.name ? (
          <span className="font-bold text-[#303030]">{item.name}</span>
        ) : (
          "this card"
        )}
      ...
      <div className="w-full flex justify-end mt-6 gap-3 font-bold text-[#303030]">
        ...
        >
          {isLoading ? <Loader2 className="w-13 animate-spin" /> : "Delete"}
        </button>
      </div>
    </Modal>
  );
